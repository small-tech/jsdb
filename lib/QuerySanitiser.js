  ...
  static sanitiseAndExecuteQueryOnData (queryString, data) {
    ...
    if (sieve === '') {
      ...
      try {
        ...
        log('Warning: syntax error in query string. Possible injection attack; rejecting.', error)
        return []
      }
        ...
        result = vm.run(`
          data.filter(function(value) {
            try {
              ...
              // (This should not happen.)
              log('Warning: query function threw unexpected error. Possible injection attack; rejecting.', error)
              return false
            }
          })
        `)
      } catch (error) /* istanbul ignore next */ {
        // There was an exception during the sandbox execution, reject.
        // (This should not happen.)
        log('Warning: query sandbox threw an exception. Possible injection attack; rejecting.')
        return []
      }

      const t3 = performance.now()

      console.log(`VM setup took  : ${(t2-t1).toFixed(3)}ms.`)
      console.log(`Query execution: ${(t3-t2).toFixed(3)}ms.`)
      console.log(`Total took     : ${(t3-t1).toFixed(3)}ms.`)
    }
