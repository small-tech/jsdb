...
class WhatTable extends EventEmitter {
  ...
  writeTableHeader () {
    this.#writeStream.write(`const _ = ${Array.isArray(this.#data) ? '[]' : '\{\}'}\n`)
    this.#writeStream.write("_.__id__ = '_'\n")
    this.#writeStream.write('module.exports = _\n')
  }
  ...
  persistChange (change) {
    ...
    console.log(`Write took ${Time.elapsed()}`)

    // Note: carry-over from full-writes. This should probably not exist.
    this.emit('save', this)
  }
}
