...
class WhatTable extends EventEmitter {
  ...
  create () {
    ...
    this._create()
    log(`   💾    ❨WhatDB?❩  ╰─ Created and persisted table in ${(performance.now() - t1).toFixed(3)} ms.`)
  }
  ...
  compact () {
    ...
    this._create()
    log(`   💾    ❨WhatDB?❩  ╰─ Compacted and persisted table in ${(performance.now() - t1).toFixed(3)} ms.`)
  }
