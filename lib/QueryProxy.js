  ...
  get cachedResult () {
    if (this.#cachedResult === null) {
      ...
      this.#cachedResult = this.data.filter(valueOf => {
        let returnValue
        try {
          returnValue = eval(this.query)
        } catch (error) {
          // Since we have no way to do type checking on the target during
          // query creation, we might end up with a (case insensitive) includes
          // operator calling .toLowerCase() on a non-string object. In that case,
          // we simply strip the offending method call and try again.
          if (error.message.includes('.toLowerCase is not a function')) {
            this.query = this.query.replace(/\.toLowerCase\(\)/g, '')
            returnValue =  eval(this.query)
          }
        }
        return returnValue
