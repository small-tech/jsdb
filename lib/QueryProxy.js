...
class QueryProxy {
  ...
  evaluateQuery () {
    ...
    for (let i = 0; i < this.cachedResult.length; i++) {
      console.log('cached: ', i, this.cachedResult[i], typeof this.cachedResult[i], isProxy(this.cachedResult[i]))
    }
  }

  getHandler (target, property, receiver) {
    console.log('QUERY PROXY GET!')
    if (property === 'get') {
