...
const onlyContainsDigitsAndDoesNotStartWithZero = value => {
  const _valueAsString = value.toString()
  return /^\d+$/.exec(value) !== null && (_valueAsString.length === 1 || _valueAsString[0] !== '0')
}

export function quoteKeyIfNotSafeInteger (key) {
  // If a key is not a safe integer, surround it in quotes.
  // Otherwise, leave it be.
  const _key = parseFloat(key)
  return onlyContainsDigitsAndDoesNotStartWithZero(key) && !isNaN(_key) && Number.isSafeInteger(_key) ? _key : `'${key}'`
}
