...
class JSDB {
  ...
  loadTables () {
    ...
    tableFiles.filter(fileName => fileName.endsWith('.js') || fileName.endsWith('.cjs')).forEach(tableFile => {
      const tableName = tableFile.replace('.js', '').replace('.cjs', '')
  ...
  setHandler (target, property, value, receiver) {
    ...
    if (!this.loadingTables) {
      ...
      const tableFileName = `${property}.${this.options.cjs ? 'c' : ''}js`
