...
export default class JSDB {
  ...
  static async open (basePath, options) {
    ...
    if (this.openDatabases[basePath] == undefined) {
      ...
      await this.openDatabases[basePath]
  ...
  // Note: constructor returns a promise.
  constructor (basePath, options = {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      console.log('this.dataProxy', this.dataProxy)
  ...
  async loadTables () {
    ...
    // Asynchronously load in the tables.
    await asyncForEach(tableFiles.filter(fileName => fileName.endsWith('.js')), async tableFile => {
      const tableName = tableFile.replace('.js', '')
      const tablePath = path.join(this.basePath, tableFile)
      const tableDataProxy = await new JSTable(tablePath)
