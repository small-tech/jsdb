...
class WhatTable {
  ...
  load() {
    console.log(`Loading table ${this.tableName}…`)
  ...
  save () {
    console.log(`Save called on table: ${this.tableName}.`)

    if (this.saveTimer && this.saveTimer.hasRef()) {
      console.log(`Ignoring save call on same event loop cycle on table: ${this.tableName}.`)
      return
    }
