...
class WhatTable {
  ...
  constructor(tablePath, rootObject = null) {
    ...
    console.log(`Table proxification took ${performance.now() - startTime} ms.`)
  ...
  deepProxy(object) {
    Object.keys(object).forEach(key => {
      ...
      if (value !== null && !isProxy(value) && typeof value === 'object') {
        // Recurse first so that we end up proxying the deepest objects first
        // and thus trigger fewer traps as we navigate the tail.
        this.deepProxy(value)
        object[key] = new Proxy(value, this.proxyHandler)
      }
    })
    return object
  }
