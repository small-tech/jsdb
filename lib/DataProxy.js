...
class DataProxy {
  ...
  static createDeepProxy (table, object) {
    Object.keys(object).forEach(key => {
      ...
      if (needsToBeProxified(value)) {
        console.log(`needs proxy:`, object[key]);
        object[key] = this.createDeepProxy(table, value)
      } else {
        console.log(`does not need proxy:`, object[key])
      }
    })
  ...
  getHandler (target, property, receiver) {
    ...
    if (property === 'where') return (function (property) {
      console.log('table, data, property', typeof this.table, this.data, property)
