...
class DataProxy {
  ...
  static createDeepProxy (table, object, proxifiedObject = null) {

    if (proxifiedObject === null) {
      proxifiedObject = Array.isArray(object) ? [] : {}
    }

    Object.keys(object).forEach(key => {
      const value = object[key]
      if (needsToBeProxified(value)) { proxifiedObject[key] = this.createDeepProxy(table, value) }
    })

    return new this(table, object, proxifiedObject)
  }
  ...
  constructor (table, data, proxifiedData) {
    this.table = table
    this.data = data
    this.proxifiedData = proxifiedData
  ...
  getHandler (target, property, receiver) {
    if (property === 'toJSON') return (function () { return this.data }).bind(this)
    ...
    return Reflect.get(this.proxifiedData, property, receiver)
  }
  ...
  setHandler (target, property, value, receiver) {
    ...
    Reflect.set(this.proxifiedData, property, value, receiver)
    this.table.save()
    return true
  }
  ...
  deletePropertyHandler (target, property) {
    Reflect.deleteProperty(this.proxifiedData, property)
